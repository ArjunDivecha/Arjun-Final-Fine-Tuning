        ...
        "formats": [
            {
                "id": "mlx",
                "name": "MLX Format",
                "description": "Native MLX adapter format (no conversion needed)"
            },
            {
                "id": "gguf",
                "name": "GGUF",
                "description": "Convert to GGUF format for llama.cpp"
            },
            {
                "id": "ollama",
                "name": "Ollama",
                "description": "Create Ollama model from adapter"
            },
            {
                "id": "lm_studio",
                "name": "LM Studio",
                "description": "Export for LM Studio"
            }
        ]
    }
...
@router.post("/export")
async def export_adapter(request: dict):
    """Export fused adapter in specified format"""
    adapter_name = request.get("adapter_name")
    format_type = request.get("format")
    output_path = request.get("output_path")
    
    if not adapter_name or not format_type:
        raise HTTPException(status_code=400, detail="adapter_name and format are required")
    
    adapter_path = os.path.join(ADAPTERS_DIR, adapter_name)
    
    logger.info(f"Export request - Adapter: {adapter_name}, Format: {format_type}, Path: {adapter_path}")
    
    if not os.path.exists(adapter_path):
        # Try to find the adapter in all subdirectories
        logger.warning(f"Adapter not found at {adapter_path}, searching...")
        found = False
        for root, dirs, files in os.walk(ADAPTERS_DIR):
            if adapter_name in dirs:
                adapter_path = os.path.join(root, adapter_name)
                logger.info(f"Found adapter at: {adapter_path}")
                found = True
                break
        
        if not found:
            raise HTTPException(status_code=404, detail=f"Adapter not found: {adapter_name} (searched in {ADAPTERS_DIR})")
    
    if format_type == "mlx":
        return {
            "success": True,
            "format": "mlx",
            "message": "Adapter is already in MLX format",
            "path": adapter_path,
            "instructions": f"Use this adapter with: python -m mlx_lm.generate --model <base_model> --adapter-path {adapter_path}"
        }
        ...
        return {
            "success": True,
            "format": "gguf",
            "message": "GGUF conversion requires llama.cpp",
            "instructions": f"""To convert to GGUF:
1. Install llama.cpp: git clone https://github.com/ggerganov/llama.cpp
2. Build: cd llama.cpp && make
3. Convert: python convert.py {adapter_path}
4. Quantize: ./quantize {adapter_path}/ggml-model-f16.gguf {adapter_path}/ggml-model-q4_0.gguf q4_0"""
        }
        ...
        if not ollama_installed:
            return {
                ...
                "instructions": "Install Ollama from https://ollama.ai"
            }
        
        return {
            "success": True,
            "format": "ollama",
            "message": "Ready to create Ollama model",
            "instructions": f"""Create Ollama model:
1. Create Modelfile in {adapter_path}
2. Run: ollama create {adapter_name} -f {adapter_path}/Modelfile
3. Run model: ollama run {adapter_name}"""
        }
    
    elif format_type == "lm_studio":
        if not output_path:
            output_path = "~/.cache/lm-studio/models"
        
        output_path = os.path.expanduser(output_path)
        dest_path = os.path.join(output_path, adapter_name)
        
        return {
            "success": True,
            "format": "lm_studio",
            "message": "Copy adapter to LM Studio",
            "path": dest_path,
            "instructions": f"""To use in LM Studio:
1. Copy adapter: cp -r {adapter_path} {dest_path}
2. Open LM Studio
3. Load base model and adapter from the models directory"""
        }
