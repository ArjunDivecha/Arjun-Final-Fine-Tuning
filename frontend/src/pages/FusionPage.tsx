...
const FusionPage: React.FC = () => {
  ...
  const [selectedExportAdapter, setSelectedExportAdapter] = useState<string>('');
      ...
      const interval = setInterval(async () => {
        try {
          ...
          if (status.status === 'completed') {
            ...
            // Set default export adapter to the fused model
            setSelectedExportAdapter(resultResponse.data.fusion_info.output_name);
  ...
  const handleExport = async (format: string) => {
    if (!fusionResult || !selectedExportAdapter) return;
    
    setExportStatus(null);
    
    try {
      const response = await axios.post('http://localhost:8000/api/fusion/export', {
        adapter_name: selectedExportAdapter,
        format: format,
        output_path: format === 'lm_studio' ? prompt('Enter LM Studio models directory:', '~/.cache/lm-studio/models') : undefined
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {fusionResult && (
        <div className="card">
          <div className="card-header">
            <h3 className="text-lg font-semibold">Export Adapter</h3>
          </div>
          <div className="card-body">
            {/* Adapter Selection */}
            <div className="mb-6">
              <label className="block text-sm font-medium mb-2">Select Adapter to Export</label>
              <select
                value={selectedExportAdapter}
                onChange={(e) => setSelectedExportAdapter(e.target.value)}
                className="input"
              >
                <option value={fusionResult.fusion_info.output_name}>
                  🎯 Fused Model: {fusionResult.fusion_info.output_name}
                </option>
                {fusionResult.individual_results.map((result) => (
                  <option key={result.adapter_name} value={result.adapter_name}>
                    📦 {result.adapter_name} (Score: {result.overall_score}/100)
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <button
                onClick={() => handleExport('mlx')}
                className="btn-primary flex flex-col items-center space-y-2 p-4 h-auto"
              >
                <Sparkles className="h-6 w-6" />
                <div className="text-center">
                  <div className="font-semibold">MLX Format</div>
                  <div className="text-xs opacity-75 mt-1">Native adapter format</div>
                </div>
              </button>

              <button
                onClick={() => handleExport('gguf')}
                className="btn-secondary flex flex-col items-center space-y-2 p-4 h-auto"
              >
                <GitMerge className="h-6 w-6" />
                <div className="text-center">
                  <div className="font-semibold">GGUF</div>
                  <div className="text-xs opacity-75 mt-1">For llama.cpp</div>
                </div>
              </button>

              <button
                onClick={() => handleExport('ollama')}
                className="btn-secondary flex flex-col items-center space-y-2 p-4 h-auto"
              >
                <Zap className="h-6 w-6" />
                <div className="text-center">
                  <div className="font-semibold">Ollama</div>
                  <div className="text-xs opacity-75 mt-1">Push to Ollama</div>
                </div>
              </button>

              <button
                onClick={() => handleExport('lm_studio')}
                className="btn-secondary flex flex-col items-center space-y-2 p-4 h-auto"
              >
                <FolderOpen className="h-6 w-6" />
                <div className="text-center">
                  <div className="font-semibold">LM Studio</div>
                  <div className="text-xs opacity-75 mt-1">Export to LM Studio</div>
                </div>
              </button>
            </div>
            ...
            {exportStatus && (
              <div className={`mt-4 p-4 rounded-lg ${
                ...
                <div className="font-semibold mb-1">{exportStatus.message}</div>
                {exportStatus.instructions && (
                  <div className="text-sm opacity-75 mt-2 font-mono">
                    {exportStatus.instructions}
                  </div>
                )}
                {exportStatus.path && (
